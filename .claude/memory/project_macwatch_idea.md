...
**Sub-modes:**
- `mw add` — scan WiFi probes + BLE, pick MAC, assign label → saved to `/apps/macwatch/watchlist.csv`
- `mw` / `mw watch` — continuous scan, beep + display when watched MAC seen

**Watchlist:** `/apps/macwatch/watchlist.csv` format: `MAC,label,WIFI|BT`. RAM fallback (3 entries) if no SD.
